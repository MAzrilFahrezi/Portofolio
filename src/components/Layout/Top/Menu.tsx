...
export default function Menu() {
  ...
  const changePage = (sectionId: string) => {
    ...
    if (activeItem instanceof HTMLElement && menuContainer.current) {
      setActiveProps({
        x:
  ...
  useEffect(() => {
    changePage(activeSection);
  }, [activeSection]);

  const isActive = (pathname: string[]) => {
    const sectionId = pathname[0]?.replace('#', '');
    return sectionId === activeSection;
  };

  const onClick = (tracking: string, sectionId: string) => {
    event('button_click', {
      page_name: activeSection,
      component_name: 'menu',
    ...
    <div className="fixed w-fit left-1/2 -translate-x-1/2 bottom-0 top-auto sm:top-0 sm:bottom-auto py-4 flex justify-center z-50">
      ...
      >
        ...
        {menu.map((item: MenuItemType, index: number) => {
          const sectionId = item.pathname[0]?.replace('#', '') || '';
          return (
            <div
              key={index}
              data-section={sectionId}
              className={classNames(
                "relative flex items-center gap-1.5 p-2 rounded-[30px] group transition duration-300 hover:bg-green-primary/50 cursor-pointer"
              )}
              onClick={() => onClick(item.tracking, sectionId)}
            >
